...
def fit_pol_all_lm(delta, data, kT=0.001, maxfun=None):
    ''' Calculate initial values for fitting and fit
...
#    trans_idx = (np.abs(dat_noslope - np.percentile(dat_noslope, 10) - sensitivity_guess/2)).argmin()
    ...
    func = lambda x, par0, par1, par2, par3, par4, par5: polmod_all_2slopes(x, np.array([par0, par1, par2, par3, par4, par5]), kT=kT)
    par_fit, pcov = scipy.optimize.curve_fit(func, delta, data, p0=par_guess)

    return par_fit, par_guess, pcov
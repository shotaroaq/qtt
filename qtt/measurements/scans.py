...
def scan2D(station, scanjob, location=None, liveplotwindow=None, plotparam='measured',
           diff_dir=None,  write_period = None, update_period = 5, verbose=1, scan_faster=False):
    ...
    for ix, x in enumerate(stepvalues):
        ...
        for iy, y in enumerate(sweepvalues):
            if scanjob['scantype'] == 'scan2Dvec':
                nskip=0
                for param in scanjob['phys_gates_vals']:
                    newvalue = scanjob['phys_gates_vals'][param][ix, iy]
                    if getattr(gates, param).get_latest()==newvalue and scan_faster:
                        if param in ['P8','P7', 'P6'] and scan_faster>=2:
                            print('ix, iy: %d %d: skipping param %s: %s' % (ix, iy, param, newvalue) )
                        nskip=nskip+1
                        pass
                    else:
                        if param in ['P8','P7', 'P6'] and scan_faster>=2:
                            print('ix, iy: %d %d: setting param %s: %s' % (ix, iy, param, newvalue) )
                        gates.set(param, scanjob['phys_gates_vals'][param][ix, iy])
                time.sleep(nskip*2*1e-3)
            else:
                sweepvalues.set(y)
            if iy == 0:
                if ix == 0:

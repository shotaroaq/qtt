...
#%%
def scan1Dfast(station, scanjob, location=None, liveplotwindow=None, delete=True, lverbose=1):
    """Fast 1D scan. 
    ...
    if scanjob['scantype'] == 'scan1Dfast':
        ...
        if 'pulsedata' in scanjob:
            waveform, sweep_info = station.awg.sweepandpulse_gate(
                ...
                scanjob['pulsedata'], delete=delete)
        else:
...
def scan2Dfast(station, scanjob, location=None, liveplotwindow=None, plotparam='measured', diff_dir=None, delete=True, verbose=1):
    ...
    if scanjob['scantype'] == 'scan2Dfastvec':
        ...
        if 'pulsedata' in scanjob:
            ...
            waveform, sweep_info = station.awg.sweep_gate_virt(
                fast_sweep_gates, sweepdata['range'], period, delete=delete)
    else:
        if 'range' in sweepdata:
            sweeprange = sweepdata['range']
        else:
        ...
        if 'pulsedata' in scanjob:
            ...
            waveform, sweep_info = station.awg.sweep_gate(
                sweepdata['param'].name, sweeprange, period, delete=delete)

...
class VideoMode:
    ...
    def __init__(self, station, sweepparams, sweepranges, minstrument, nplots=None, Naverage=10,
                 ...
                 dorun=True, show_controls=True, add_ppt=True, crosshair=False, averaging=True, delete=True):
        self.station = station
    ...
    def run(self, startreadout=True, delete=True):
        ...
        if dim == 1:
            ...
            if isinstance(self.sweepparams, list):
                ...
                ), self.sweepparams, self.sweepranges, self.resolution, delete=delete)
            elif isinstance(self.sweepparams, dict):
                waveform, _ = awg.sweep_2D_virt(self.sampling_frequency.get(), self.sweepparams[
                                                             'gates_horz'], self.sweepparams['gates_vert'], self.sweepranges, self.resolution, delete=delete)
            else:
                raise Exception('arguments not supported')
